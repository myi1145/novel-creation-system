...
class GateService:
    ...
    def run_single_gate(self, db: Session, project_id: str, draft_id: str, gate_name: str, workflow_run_id: str | None = None, trace_id: str | None = None) -> GateReviewResult:
        ...
        genre_context = rulepack_service.resolve_context(db=db, project_id=project_id)
        result = self._evaluate_gate(db=db, gate_name=gate_name, draft=draft, blueprint=blueprint, latest_snapshot=latest_snapshot, genre_context=genre_context, run_id=run.id, trace_id=run.trace_id)
        ...
        return _to_gate_review_schema(review)
    ...
    def _evaluate_gate(self, db: Session, gate_name: str, draft: ChapterDraftORM, blueprint: ChapterBlueprintORM | None, latest_snapshot: CanonSnapshotORM | None, genre_context, run_id: str, trace_id: str) -> GateReviewResult:
        ...
        gate_constraints = genre_context.gate_constraints
        ...
        if content:
            for term in list(gate_constraints.banned_terms or []):
                if term and term.lower() in lowered_content:
                    issues.append(self._issue(severity="S2", message=f"正文命中题材禁用词：{term}", suggestion="替换或删除与题材风格冲突的术语", category="genre_rulepack", summary="触发题材禁用词", evidence_refs=[f"draft:{draft.id}"]))
            if gate_name in {"canon_gate", "style_gate", "publish_gate"}:
                for taboo in list(gate_constraints.taboos or []):
                    if taboo and taboo.lower() in lowered_content:
                        issues.append(self._issue(severity="S2", message=f"正文触发题材禁忌：{taboo}", suggestion="移除违反题材设定的表达或事件", category="genre_rulepack", summary="触发题材禁忌", evidence_refs=[f"draft:{draft.id}"]))
            if gate_name in {"narrative_gate", "publish_gate"} and gate_constraints.chapter_target_words:
                target_words = max(int(gate_constraints.chapter_target_words), 1)
                if len(content) < max(120, int(target_words * 0.35)):
                    issues.append(self._issue(severity="S2", message=f"正文长度明显低于题材建议章节字数目标（目标约 {target_words}）", suggestion="补足章节推进、场景承接与题材要求的篇幅", category="genre_rulepack", summary="低于题材章节目标", evidence_refs=[f"draft:{draft.id}"]))
            if gate_name == "narrative_gate" and gate_constraints.preferred_conflict_types:
                blueprint_text = " ".join((blueprint.advances if blueprint is not None else []) or []) + " " + (blueprint.summary if blueprint is not None else "")
                signal_text = f"{blueprint_text} {content}".lower()
                preferred_hits = [item for item in gate_constraints.preferred_conflict_types if item and item.lower() in signal_text]
                if not preferred_hits:
                    issues.append(self._issue(severity="S1", message="本章未明显体现题材偏好冲突类型", suggestion="检查蓝图与正文是否承接题材默认冲突模式", category="genre_rulepack", summary="题材冲突承接偏弱", evidence_refs=[f"draft:{draft.id}", f"blueprint:{blueprint.id}" if blueprint is not None else f"draft:{draft.id}"]))
        ...
        if content:
            review_result = agent_gateway.review_gate(
                ...
                context={
                    ...
                    "genre_banned_terms": list(gate_constraints.banned_terms or []),
                    "genre_taboos": list(gate_constraints.taboos or []),
                    "canon_summary": {

...
class MockAgentProvider:
    ...
    def generate_story_planning(self, context: dict[str, Any], prompt: PromptTemplateResolution) -> dict[str, Any]:
        ...
        return {
            "worldview": (
                f"[世界背景] 项目《{project_name}》基于{genre_name}题材展开，围绕“{premise}”建立长期连载背景。\n"
                "[力量体系] 力量成长遵循阶梯制与反制关系，任何突破都需成本与后果。\n"
                "[社会秩序] 既有秩序与灰色地带并存，主角行动会触发制度性反馈。\n"
                "[势力格局] 守成势力、扩张势力、隐性操盘势力三方长期博弈。\n"
                "[资源与代价] 资源稀缺与代价递增共同驱动冲突升级。\n"
                "[隐藏真相方向] 世界深层真相分阶段揭示，不允许前期一次性揭底。\n"
                "[规则边界] 禁止临时改规则、禁止无代价升级、禁止越权写正史。"
            ),
            "main_outline": (
                f"[阅读承诺] 提供“{premise}”的持续兑现：每卷都有阶段目标达成与更大代价。\n"
                "[主角长期成长主线] 主角从被动求生成长为主动破局者，能力、认知、责任同步升级。\n"
                "[核心冲突] 主角目标与既有秩序发生结构性冲突，并逐卷放大。\n"
                "[关键角色关系张力] 主角与盟友/导师/对手在信任与利益上持续拉扯。\n"
                "[关键配角功能] 配角分别承担镜像、催化、制衡、背叛与救赎功能。\n"
                "[主要对抗力量] 显性敌对势力负责压迫，隐性操盘者负责制造误导。\n"
                "[感情线/情绪主线] 情绪曲线按压抑→对抗→决断→反噬递进。\n"
                "[长期钩子] 贯穿全书的核心谜题：主角命运与世界真相是否同源。\n"
                "[主线架构] 前期入局→中期扩张→中后期升级→终局收束。\n"
                "[关键转折点] 每个阶段都设置高代价选择，迫使关系与立场重排。\n"
                f"[长程悬念问题] 终局前持续追问：主角拯救的是世界，还是被世界塑造成新秩序（基调：{tone}）。"
            ),
            "volume_plan": (
                f"[分卷规划原则] 分卷总章节参考：{target_chapter_count}；每卷需承担职责、目标、冲突、推进、卷末转折。\n"
                "[卷一职责] 入局卷：职责=建立规则与关系网；目标=完成立足；冲突=首轮秩序压迫；关键推进=主角获得第一份主动权；卷末转折=发现更高层敌意。\n"
                "[卷二职责] 扩张卷：职责=扩大战场并重排关系；目标=从局部胜利迈向系统对抗；冲突=多势力围猎；关键推进=主角阵营成形；卷末转折=核心盟友立场动摇。\n"
                "[卷三职责] 收束卷：职责=揭示真相并完成价值抉择；目标=解决主线矛盾；冲突=显性与隐性对抗合流；关键推进=终局决策；卷末转折=留下可连载后续钩子。\n"
                f"[卷末承接] 各卷结尾均保留下一卷驱动问题，叙事基调维持{tone}。"
            ),
            "core_seed_summary": (
                "[核心种子] 角色/势力/地点/术语四类种子均绑定主线冲突，不做孤立设定。\n"
                "[初始状态快照] 开局时主角弱势、关系未稳、外部秩序压制、伏笔已埋。\n"
                "[主角初始状态] 资源稀缺、认知局限，但具备打破僵局的潜在触发点。\n"
                "[关键关系初始状态] 与导师互相利用、与搭档脆弱信任、与对手敌意未明。\n"
                "[已知开放问题] 主角为何被盯上、关键势力真实意图、失落历史空白段。\n"
                "[埋下的谜团/伏笔] 身份谜团、规则漏洞、旧案残片三条线并行推进。\n"
                "[开局局势张力] 时间压力+资源短缺+敌对追索共同作用，迫使主角快速入局。\n"
                "[前期不可随意改写的状态边界] 主角起点身份、核心关系基线、首卷关键伏笔禁止回写篡改。"
            ),
        }

...
class ContinuityService:
    ...
    def resolve_pack(self, db: Session, request: ResolveContinuityPackRequest, *, commit: bool = True) -> ContinuityPack:
        ...
        if project.genre_id and genre_context.genre_name == project.genre_id:
            warnings.append("项目已配置 genre_id，但尚未加载正式 GenreProfile，当前使用通用规则包。")
        ...
        if genre_context.rulepack_summary:
            continuity_summary_parts.append(f"题材规则：{genre_context.rulepack_summary}")
        ...
        workflow_run_service.update_progress(
            ...
            extra_metadata={
                ...
                "genre_name": pack.genre_name,
                "genre_rulepack_summary": pack.genre_rulepack_summary,
            },
        )
        db.add(
            ImmutableLogORM(

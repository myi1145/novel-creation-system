...
当前仓库阶段口径以 handoff 为准：发布前一致性验收 / 章节发布准入与作者端术语收口阶段已签收完成，当前进入“章节发布历史与版本追踪”的方向确认与边界立项阶段；相关决策链位于 `md/next_stage_decision_publish_history/`（00~04），并由 handoff 统一收口。
...
- 当前阶段（以 `md/status/current_stage_handoff.md` 为准）为“章节发布历史与版本追踪立项（docs-only）”；工程能力建设仍按既有基线持续演进，不应高估为 fully autonomous production system；

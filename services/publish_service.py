        ...
        try:
            ...
            if publish_gate_review.pass_status == "failed":
                chapter_state_service.transition(
                    ...
                    trace_id=run.trace_id,
                    reason="发布 Gate 未通过",
                    metadata={"highest_severity": publish_gate_review.highest_severity},
                )
                workflow_run_service.mark_attention(db=db, run=run, current_step="publish_gate_failed", reason="发布 Gate 未通过")
                db.commit()

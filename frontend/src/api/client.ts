...
export type ObjectResource = 'characters' | 'rules' | 'open-loops' | 'relationships';
...
export const api = {
  ...
  listObjects: (resource: ObjectResource, projectId: string) => http.get<CreativeObject[]>(`/objects/${resource}?project_id=${projectId}`),
  objectHistory: (resource: ObjectResource, projectId: string, logicalObjectId: string) =>
    http.get<CreativeObject[]>(`/objects/${resource}/history?project_id=${projectId}&logical_object_id=${logicalObjectId}`),
  proposeObjectUpdate: (resource: ObjectResource, logicalObjectId: string, payload: Dict) =>
    http.post<ChangeSet>(`/objects/${resource}/${logicalObjectId}/changesets/update`, payload),
  proposeObjectRestore: (resource: ObjectResource, logicalObjectId: string, payload: Dict) =>
    http.post<ChangeSet>(`/objects/${resource}/${logicalObjectId}/changesets/restore`, payload),
  proposeObjectRetire: (resource: ObjectResource, logicalObjectId: string, payload: Dict) =>

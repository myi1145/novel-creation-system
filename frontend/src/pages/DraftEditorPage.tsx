...
export function DraftEditorPage() {
  ...
  const onSave = async (event: FormEvent) => {
    ...
    if (!trimmedEditReason) {
      setError('请填写“人工修订原因”，用于后续质量检查和变更提案判断本次改动背景。');
      return;
    }
    ...
    try {
      ...
      setFeedback('保存成功：已写回人工修订草稿。下一步建议回质量检查重新审查，再继续变更提案 → 发布章节。');
    } catch (e) {
    ...
    <div>
      ...
      <div className="panel">
        <div className="project-nav">
          <Link to={`/projects/${projectId}/gates`}>下一步：回质量检查重新审查</Link>
          <Link to={`/projects/${projectId}/changesets`}>然后：去变更提案</Link>
          <Link to={`/projects/${projectId}/published`}>最后：去发布章节</Link>

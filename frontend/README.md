...
- 当前阶段边界：
  - 支持“生成章节目录”草稿：基于已保存的全书规划生成目录标题、摘要与章节项草稿。
  - 生成结果仅回填编辑表单，作者仍需手动点击“保存章节目录”才会写库。
  - 候选卡仅在作者手动触发“生成候选卡”后创建。
  - 当前不会自动生成卡槽候选。
  - 当前不会接入 ChapterGoal / ChapterBlueprint。
  - 不写 Canon。
  - 不生成 ChangeSet。
...
四类基础卡槽（角色卡、术语卡、势力卡、地点卡）现已支持：

- JSON 全量导出（四类一起导出）与 JSON 全量导入。
- CSV 单类导出与 CSV 单类导入。
- CSV 模板下载（每类固定表头 + 示例行）。
- 导入结果报告（created / skipped / errors）。

固定策略与边界：

- 重复名称默认 `skip_duplicates`（同项目内同名自动跳过并进入 skipped 报告）。
- 导入仅写入四类基础卡槽工作态数据。
- 导入不会写 Canon。
- 导入不会生成 ChangeSet。
- 当前不支持 Excel 导入导出。
...
- 现支持「生成全书规划」草稿（仅生成，不自动保存）：
  - 点击后调用后端生成接口，自动回填四个编辑区。
  - 生成成功后仍需作者检查、修改并点击“保存全书规划”才会写库。
- 四个编辑区：
  - 全书世界观
  - 全书主线大纲
  - 分卷 / 阶段规划
